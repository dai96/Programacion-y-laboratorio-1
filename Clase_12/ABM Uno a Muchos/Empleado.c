...
void hardcodearDatosEmpleados(eEmpleado lista[], int tam)
{
    ...
    char nombres[][50]= {"Daira","Pedro","Sofia","Belen","Julian","Micaela"};
    char sexo[]= {'F','M','F','F','M','F'};
    float sueldosBruto[]= {22000,22000,15000,4000,21000,6000};
    int sector[]={2,2,3,1,1,2};
    //char direciones[]={"castelli 22"
...
void ordenarVectorAlfabeticamente(eEmpleado lista [], int tam)//Burbugeo
{
    int i,j;
    eEmpleado aux;
    for (i=0;i<tam-1;i++)
    {
        for (j=i+1;j<tam;j++)
        {
            if(strcmp(lista[i].nombre,lista[j].nombre)>0)//criterio
            {
                aux=lista[i];
...
/*int carcargsectores(eSector lista[], int tam, eEmpleado lista[], int tamE)
{
    for (i=0;i<tamE;i++)
    {
